...
from abstractions.defaults import OPTIONAL_PACKAGES, auth_app_name
...
class WriteAuth(TerminalController):
    ...
    def __init__(self, project):
        ...
        super().__init__(project.project_path, project.formatted_name, False, project.id)
    ...
    def setup_auth(self):
        send_process_message(self.project.owner.id, "creating f{auth_app_name} app...")
        self.create_app(auth_app_name)
        
        send_process_message(self.project.owner.id, "creating necessary models...")
        self.create_auth_model()
        
        send_process_message(self.project.owner.id, "creating auth utilities...")
        self.create_auth_utitlties()
        self.create_auth_permission()
        
        send_process_message(self.project.owner.id, "creating auth serializers...")
    ...
    def create_auth_model(self):
        self.custom_username = f"{self.formatted_name.title()}CustomUser"
        # define imports
        ...
        has_slug, slug_data, data  = WriteToModel.handle_model_fields(fields, data_content, self.get_formatted_name)
        data_content = data
        ...
        data_content = WriteToModel.format_string_representation(data_content, [{self.username_field}]) + "\n"
        data_content = WriteToModel.format_meta(data_content, {'ordering': "('created_at', )"}) + "\n"
        
        if has_slug:
            data_content = WriteToModel.format_slug(False, data_content, slug_data)
    ...
    def create_auth_utitlties(self):
        ...
        data_content += f"\treturn jwt.encode(\{'exp': datetime.now() + timedelta(minutes={self.project_auth.access_expiry}), **payload\}, settings.SECRET_KEY, algorithm='HS256')\n\n"
        
        data_content += f"def get_refresh_token():\n"
        data_content += f"\treturn jwt.encode(\{'exp': datetime.now() + timedelta(days={self.project_auth.refresh_expiry}), 'data': get_random(10)\}, settings.SECRET_KEY, algorithm='HS256')\n\n"
        ...
        data_content += f"\t\ttry:\n"
        data_content += f"\t\t\treturn {self.custom_username}.objects.get(id=decode['user_id'])\n"
        data_content += f"\t\texcept Exception:\n"
        data_content += f"\t\t\treturn None\n"
    ...
    def create_auth_permission(self):
        ...
        data_content += "from django.utils import timezone\n"
        data_content += "from django.contrib.auth import authenticate\n\n\n"
        ...
        data_content += f"\t\trquest.user = user\n"
    ...
    def create_serializer(self):
        ...
        fields = fields = self.project_auth.properties["fields"] or []
    ...
    def create_views(self):
        ...
        data_content += f"import re\n"
        data_content += f"from django.db.models import Q, Count, Subquery, OuterRef\n"
        ...
        data_content += f"\t\tCustomUser.objects.create_user({self.username_field}={self.username_field}, **serializer.validated_data)\n\n"
        ...
        data_content += f"\t\t\treturn Response({'error': 'refresh token not found'}, status='400')\n\n"
        
        data_content += f"\t\tif not verify_token(serializer.validated_data['refresh']):\n"
        data_content += f"\t\t\treturn Response({'error': 'Token is invalid or has expired'}, status='400')\n\n"
    ...
    def create_urls(self):
        # define imports
        data_content = "from django.urls import path, include\n"
