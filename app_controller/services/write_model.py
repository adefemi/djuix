...
class WriteToModel(WriterMain):
    ...
    def write_model(self):
        ...
        for model in self.models:
            ...
            has_slug, slug_data, data = self.handle_model_fields(fields, self.content_data)
            self.content_data = data

            has_created_at = field_properties.get('has_created_at', False)
            if has_created_at:
                self.content_data = self.format_has_created_date(
                    self.content_data)

            has_updated_at = field_properties.get('has_updated_at', False)
            if has_updated_at:
                self.content_data = self.format_has_updated_date(
                    self.content_data)
            ...
            # if string_rep:
            #     self.content_data = self.format_string_representation(
            #         self.content_data, string_rep)

            if has_slug:
                self.content_data = self.format_slug(
                    has_save_method, self.content_data, slug_data)
    ...
    @staticmethod
    def handle_model_fields(fields, data):
        has_slug = False
        slug_data = None
        for field_data in fields:
            ...
            if field_attrs:
                related_model_name = field_attrs.pop(
            ...
            data += f"\t{field_data['name']} = models.{field_data['field_type']}({attrs_string})\n"
