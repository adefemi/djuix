...
class GenerateModelComponents:
    ...
    def generate_model_serializer(self):
        serializer_name = self.model.name.replace('Model', 'Serializer')
        ...
        for field in model_fields:
            if field["field_type"] in (ModelFieldTypes.ForeignKey, ModelFieldTypes.OneToOneField):
                a = {
                    "name": field["name"],
                    "field_type": field["field_properties"]["related_model_name"].replace('Model', 'Serializer'),
                    "is_custom_type": True,
                    "field_properties": {
                        "read_only": "True",
                    }
        ...
        self.serializer = SerializerInfo.objects.get(
            id=serializer_v.data["id"])

    def generate_model_view(self):
        view_name = self.model.name.replace('Model', 'View')
    ...
    def get_view_url(self):
        url_name = Helper.camelToSnakeDash(self.view.name.replace('View', 'Url'))
